...
export default function Hero() {
  ...
  const currentRef = useRef({ x: 0, y: 0, nx: 0, ny: 0, px: 0, py: 0 })
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Smooth lerp — different speeds for each layer
      c.x  += (t.x * 14 - c.x)  * 0.05   // name: ±14px
      c.y  += (t.y * 9  - c.y)  * 0.05
      c.nx += (t.x * 7  - c.nx) * 0.06   // mid: ±7px
      c.ny += (t.y * 5  - c.ny) * 0.06
      c.px += (t.x * 3  - c.px) * 0.07   // pills: ±3px
      c.py += (t.y * 2  - c.py) * 0.07

      if (nameRef.current)
        nameRef.current.style.transform = `translate(${c.x.toFixed(2)}px, ${c.y.toFixed(2)}px)`
      if (midRef.current)
        midRef.current.style.transform = `translate(${c.nx.toFixed(2)}px, ${c.ny.toFixed(2)}px)`
      if (pillsRef.current)
        pillsRef.current.style.transform = `translate(${c.px.toFixed(2)}px, ${c.py.toFixed(2)}px)`
  ...
  return (
    <section ref={sectionRef} className="relative min-h-screen flex items-center justify-center pt-14 overflow-hidden">
      {/* Radial glow */}
      ...
      <div className="relative z-10 text-center px-6 w-full max-w-3xl mx-auto" style={{ transform: 'translateY(-5vh)' }}>
        ...
        <div ref={midRef} style={{ willChange: 'transform' }}>
          ...
          <div className="flex items-center justify-center gap-4 mb-10 fade-up fade-up-delay-5">
            <TerminalButton href="#projects" primary>VIEW PROJECTS</TerminalButton>
            <TerminalButton href="#contact">CONTACT</TerminalButton>
            <TerminalButton href="/resume.pdf" external>RESUME</TerminalButton>
          </div>
        </div>

        {/* Stat pills — parallax layer: back (moves the least) */}
