...
const statusColor = { Live: 'var(--ok)', 'In Development': 'var(--warn)', Ongoing: 'var(--accent-bright)', Completed: 'var(--text-muted)' }
...
function ProjectCard({ project, onUpdate, onRemove }) {
  ...
  return (
    <div style={{
      background: 'var(--surface-sunken)', border: '1px solid var(--border-subtle)', borderRadius: 8, padding: 20,
    }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
        <div>
          <label style={{ fontFamily: 'JetBrains Mono', fontSize: 10, color: 'var(--text-faint)', display: 'block', marginBottom: 5 }}>NAME</label>
        ...
        <div>
          <label style={{ fontFamily: 'JetBrains Mono', fontSize: 10, color: 'var(--text-faint)', display: 'block', marginBottom: 5 }}>STATUS</label>
      ...
      <div style={{ marginBottom: 12 }}>
        <label style={{ fontFamily: 'JetBrains Mono', fontSize: 10, color: 'var(--text-faint)', display: 'block', marginBottom: 5 }}>DESCRIPTION</label>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
        <div>
          <label style={{ fontFamily: 'JetBrains Mono', fontSize: 10, color: 'var(--text-faint)', display: 'block', marginBottom: 5 }}>GITHUB URL</label>
          <input value={s.github || ''} onChange={e => onUpdate('github', e.target.value)}
            style={{ width: '100%', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 4, padding: '8px 12px', color: 'var(--text)', fontSize: 12, fontFamily: 'JetBrains Mono', outline: 'none' }} />
        </div>
        <div>
          <label style={{ fontFamily: 'JetBrains Mono', fontSize: 10, color: 'var(--text-faint)', display: 'block', marginBottom: 5 }}>LIVE URL</label>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
        <div>
          <label style={{ fontFamily: 'JetBrains Mono', fontSize: 10, color: 'var(--text-faint)', display: 'block', marginBottom: 5 }}>SCREENSHOT PATH</label>
      ...
      <div style={{ marginBottom: 12 }}>
        <label style={{ fontFamily: 'JetBrains Mono', fontSize: 10, color: 'var(--text-faint)', display: 'block', marginBottom: 5 }}>CASE STUDY</label>
      ...
      {/* Stack */}
      <div style={{ marginBottom: 12 }}>
        <label style={{ fontFamily: 'JetBrains Mono', fontSize: 10, color: 'var(--text-faint)', display: 'block', marginBottom: 8 }}>STACK</label>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>
          {s.stack.map(tech => (
            <span key={tech} style={{
              background: 'rgba(var(--accent-rgb), 0.07)', border: '1px solid rgba(var(--accent-rgb), 0.15)',
              borderRadius: 3, padding: '3px 8px', fontFamily: 'JetBrains Mono', fontSize: 10, color: 'var(--text-muted)',
              display: 'flex', alignItems: 'center', gap: 6,
            }}>
              {tech}
              <button onClick={() => onUpdate('stack', s.stack.filter(t => t !== tech))}
                style={{ background: 'none', border: 'none', color: 'var(--text-faint)', cursor: 'pointer', fontSize: 12, padding: 0, lineHeight: 1 }}>×</button>
            </span>
          ))}
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <input value={stackInput} onChange={e => setStackInput(e.target.value)}
            ...
            style={{
              background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 4, padding: '7px 12px',
              color: 'var(--text)', fontSize: 12, fontFamily: 'JetBrains Mono', outline: 'none', flex: 1,
            }} />
        </div>
      </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {adding ? (
        <div style={{ background: 'var(--surface-sunken)', border: '1px dashed var(--accent)', borderRadius: 8, padding: 20 }}>
          <p style={{ fontFamily: 'JetBrains Mono', fontSize: 10, color: 'var(--accent)', marginBottom: 16 }}>NEW PROJECT</p>
          <ProjectCard
            project={draft}
